...
/////////////////////////////////////////////////////////////////////////
///주어진 너비(width)와 종횡비(aspectRatio)를 통해 높이를 구하는 utils 함수입니다.///
///////////////////////////////////////////////////////////////////////
const calculateHeight = (width, aspectRatio) => {
  const ratio = aspectRatio.split(":").map(Number);
  const [numerator, denominator] = ratio;
  const decimalRatio = denominator / numerator;
  return width * decimalRatio;
};
///////////////////////////////////////////////
...
const useValidation = initialState => {
  ...
  const checkValidation = useCallback((name, value, body) => {
    setValid(prevValid => {
      ...
      if (name === "password") {
        return { ...prevValid, [name]: isValid, ["password2"]: value === body.password2 };
      }
