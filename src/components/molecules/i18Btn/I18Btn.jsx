...
const I18Btn = () => {
  ...
  const getCurrentLanguageImage = () => {
    ...
    const selectedImage = LanguageCountryImage.find((item) => item.id === currentLanguageId);
    return selectedImage ? selectedImage.src : '';
  };

  return (
    <LocaleContainer>
      <figure className={`locale-btn ${open ? 'active' : ''} icon-btn`} onClick={onOpen}>
        <CustomImage image={getCurrentLanguageImage()} alt="current-country" />
        <p className="arrow" />
      </figure>
      <div className={`locale-bg ${open ? 'open' : 'close'}`} onMouseOver={onClose} onFocus={onClose} />
      ...
      <div className={`locale-select-box ${open ? 'open' : 'close'}`}>
        {LanguageCountryImage?.map((locale) => (
          <Link href={router?.asPath || ''} locale={locale?.id} key={`locale-${locale?.id}`}>
            <CustomImage className={t('id') === locale?.id ? 'active-btn' : ''} image={locale?.src} alt="language-country" />
          </Link>
        ))}
      </div>
    </LocaleContainer>
